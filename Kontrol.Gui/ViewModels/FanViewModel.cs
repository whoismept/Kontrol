...
public partial class FanViewModel : ObservableObject, IDisposable
{
    ...
    public ObservableCollection<DashboardDeviceItem> DashDevices { get; } = new();
    ...
    private void UpdateLiveSummary(List<SensorReading> readings)
    {
        ...
        UpdateDashDevices(readings, temps, fans);
    }

    private void UpdateDashDevices(List<SensorReading> readings,
                                    List<SensorReading> temps,
                                    List<SensorReading> fans)
    {
        if (!_dashDevicesInitialized)
        {
            _dashDevicesInitialized = true;
            foreach (var info in _hardwareService.GetHardwareInfos())
                DashDevices.Add(new DashboardDeviceItem { Name = info.Name, TypeLabel = info.TypeLabel });
        }

        // GPU hardware names — used so CPU fallback excludes GPU fans
        var gpuNames = DashDevices
            .Where(d => d.TypeLabel == "GPU")
            .Select(d => d.Name)
            .ToHashSet(StringComparer.OrdinalIgnoreCase);

        foreach (var item in DashDevices)
        {
            var deviceTemps = temps
                .Where(r => r.HardwareName.Equals(item.Name, StringComparison.OrdinalIgnoreCase))
                .ToList();
            var maxTemp = deviceTemps.Select(r => r.Value).DefaultIfEmpty(0).Max();
            item.MaxTempText = maxTemp > 0 ? $"{maxTemp:F0}°C" : "—";

            List<SensorReading> deviceFans;
            if (item.TypeLabel == "GPU")
            {
                deviceFans = fans
                    .Where(r => r.HardwareName.Equals(item.Name, StringComparison.OrdinalIgnoreCase))
                    .ToList();
            ...
            {
                // Try: sensors directly under CPU hardware OR named "CPU*"
                deviceFans = fans
                    .Where(r => r.HardwareName.Equals(item.Name, StringComparison.OrdinalIgnoreCase)
                             || r.SensorName.Contains("CPU", StringComparison.OrdinalIgnoreCase))
                    .ToList();

                // Fallback: CPU cooler fans often live under SuperIO/Motherboard without "CPU" label.
                // Use all fans that don't belong to a detected GPU.
                if (deviceFans.Count == 0)
                    deviceFans = fans.Where(r => !gpuNames.Contains(r.HardwareName)).ToList();
            }

            var maxRpm = deviceFans.Select(r => r.Value).DefaultIfEmpty(0).Max();
            item.FanRpmText = maxRpm > 0 ? $"{maxRpm:F0} RPM" : "—";
            item.FanRpm     = maxRpm;
            item.FanCount   = deviceFans.Select(r => r.SensorName).Distinct().Count();
        }
    }
